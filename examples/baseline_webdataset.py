...
"""
Training ResNet50 on Imagenet according to: https://arxiv.org/pdf/1706.02677.pdf
"""
...
parser.add_argument(
    "--lr",
    "--basic-learning-rate (256)",
    default=0.1,
    type=float,
    metavar="LR",
    help="learning rate for bs = 256",
    dest="lr",
)
...
parser.add_argument(
        '--trainshards', 
        default='gs://imagenet-tar/shards/imagenet-train-{000000..000146}.tar', 
...
def main():
    ...
    dist.init_process_group(
        backend=args.dist_backend, init_method="env://", timeout=timedelta(seconds=300)
    )

    print("-----initialize model------")
    
    lr = args.lr * (int(os.environ["WORLD_SIZE"]) * args.batch_size)/256 # TODO: check that the global batch size is correct
    model, criterion, optimizer = initialize_model(
        args.arch, lr, args.momentum, args.weight_decay, device_id
    )

    print("----------- init data loader---------")
    train_loader, val_loader = initialize_data_loader(
        args.data, args.batch_size, args.workers, args.trainshards, int(os.environ["WORLD_SIZE"])
    )


    print("----- World size is: ", int(os.environ["WORLD_SIZE"]), ", target learning rate is: ", lr)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        writer.add_scalar("Accuracy-epoch/validation", acc1, epoch)
...
def initialize_model(
    ...
    criterion = nn.CrossEntropyLoss().cuda(device_id) # TODO: normalize by total minibatch size (section 3)
    optimizer = SGD(
...
def initialize_data_loader(
    data_dir, batch_size, num_data_workers, trainshards, world_size
    ...
    train_loader = wds.WebLoader(
        train_dataset, batch_size=None, shuffle=False, num_workers=num_data_workers,
    )
...
def train(
    ...
    for i, (images, target) in enumerate(train_loader):
        ...
        print("forward took: ", time.time()-st)
        et = time.time()

        #loss = loss/int(os.environ['WORLD_SIZE']) * bs # TODO: check this normalization
        ...
        optimizer.zero_grad()
        loss.backward()
        print("backward took: ", time.time()-et)

        ft = time.time()
        optimizer.step()
        print("step took: ", time.time()-ft)

        print("Iteration ", i, " Data loading took: ", dl, " Training took: ", time.time()-st)
...
def adjust_learning_rate(optimizer, epoch: int, lr: float) -> None:
    ...
    """
    if epoch < 5:
        learning_rate = lr * ((epoch+1)/5)
    else:
        learning_rate = lr * (0.1 ** (epoch // 30))

    print("---- Learning rate at epoch ", epoch, " is: ", learning_rate)    
